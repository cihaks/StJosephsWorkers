...
module RoleRequirementSystem
  def self.included(klass)
    ...
    klass.send :helper_method, :authorized_for? 
  ...
  module RoleSecurityInstanceMethods
    ...
    def render_optional_error_file(status)
      flash[:error] = "You do not have rights to use that resource"
  ...
  protected
    ...
    def authorized_for?(action = nil, controller = @controller)
      url_options_authenticate?({:controller=>controller, :action=>action})
    end
  end
end